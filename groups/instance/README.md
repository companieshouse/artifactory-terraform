...
| [aws_iam_role_policy_attachment.cloudwatch_managed](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| [aws_iam_policy.cloudwatch_managed](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy) | data source |
...
| <a name="input_artifactory_base_path"></a> [artifactory\_base\_path](#input\_artifactory\_base\_path) | The base install path for Artifactory | `string` | `"/opt/jfrog/artifactory"` | no |
| <a name="input_artifactory_config_server_name"></a> [artifactory\_config\_server\_name](#input\_artifactory\_config\_server\_name) | The full display name for the Artifactory server instance | `string` | n/a | yes |
| <a name="input_artifactory_group"></a> [artifactory\_group](#input\_artifactory\_group) | The system group that the Artifactory service user is a member of | `string` | `"artifactory"` | no |
| <a name="input_artifactory_user"></a> [artifactory\_user](#input\_artifactory\_user) | The system user that the Artifactory service runs as | `string` | `"artifactory"` | no |
...
| <a name="input_asg_health_check_grace_period"></a> [asg\_health\_check\_grace\_period](#input\_asg\_health\_check\_grace\_period) | Time (in seconds) after instance comes into service before checking health. Default 150 | `number` | `150` | no |
...
| <a name="input_aws_command"></a> [aws\_command](#input\_aws\_command) | The base aws cli get-parameter command used during instance bootstrap | `string` | `"aws ssm get-parameter --with-decryption --output text"` | no |
| <a name="input_cloudwatch_log_group_retention_in_days"></a> [cloudwatch\_log\_group\_retention\_in\_days](#input\_cloudwatch\_log\_group\_retention\_in\_days) | The number of days to retain cloudwatch logs | `number` | `14` | no |
| <a name="input_cloudwatch_logs_collected"></a> [cloudwatch\_logs\_collected](#input\_cloudwatch\_logs\_collected) | A list of objects that contain key=value pairs, used to generate cloudwatch agent config for collecting application logs | <pre>list(object(<br/>    {<br/>      name             = string<br/>      timestamp_format = optional(string, "%Y-%m-%dT%H:%M:%S.%f")<br/>    }<br/>  ))</pre> | `[]` | no |
...
| <a name="input_ebs_root_delete_on_termination"></a> [ebs\_root\_delete\_on\_termination](#input\_ebs\_root\_delete\_on\_termination) | Whether the volume should be destroyed on instance termination. Defaulted to false | `bool` | `true` | no |
| <a name="input_ebs_root_iops"></a> [ebs\_root\_iops](#input\_ebs\_root\_iops) | EBS volume IOPS; 3000 is the gp3 default | `number` | `3000` | no |
| <a name="input_ebs_root_throughput"></a> [ebs\_root\_throughput](#input\_ebs\_root\_throughput) | EBS volume throughput in MiB/s; 125 is the gp3 default | `number` | `125` | no |
...
| <a name="input_ebs_root_volume_type"></a> [ebs\_root\_volume\_type](#input\_ebs\_root\_volume\_type) | EBS volume type to provision | `string` | `"gp3"` | no |
...
| <a name="input_lvm_block_devices"></a> [lvm\_block\_devices](#input\_lvm\_block\_devices) | A list of objects representing LVM block devices; each LVM volume group is assumed to contain a single physical volume and each logical volume is assumed to belong to a single volume group; the filesystem for each logical volume will be expanded to use all available space within the volume group using the filesystem resize command specified; block device configuration applies only on resource creation | <pre>list(object({<br/>    aws_volume_size_gb : string,<br/>    delete_on_termination : bool,<br/>    filesystem_resize_command : string,<br/>    lvm_logical_volume_device_node : string,<br/>    lvm_physical_volume_device_node : string,<br/>  }))</pre> | n/a | yes |
...
| Name | Description |
|------|-------------|
| <a name="output_alb_dns_name"></a> [alb\_dns\_name](#output\_alb\_dns\_name) | Returns the AWS DNS name for the ALB |
| <a name="output_alb_name"></a> [alb\_name](#output\_alb\_name) | Returns the name of the deployed ALB |
| <a name="output_alb_security_group_id"></a> [alb\_security\_group\_id](#output\_alb\_security\_group\_id) | Returns the id of the security group attached to the ALB |
| <a name="output_asg_name"></a> [asg\_name](#output\_asg\_name) | Returns the name of the deployed ASG |
| <a name="output_asg_security_group_id"></a> [asg\_security\_group\_id](#output\_asg\_security\_group\_id) | Returns the id of the security group attached to ASG EC2 instances |
<!-- END_TF_DOCS -->
